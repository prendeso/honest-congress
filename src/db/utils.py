...
from src.db import Member, Disclosure, Anomaly
...
def update_all_member_counts(db: Session) -> dict:
    ...
    db.query(Member).filter(
        ~Member.id.in_([m_id for m_id, _ in disclosure_counts])
    ).update({"disclosure_count": 0}, synchronize_session=False)
    ...
    db.query(Member).filter(
        ~Member.id.in_([m_id for m_id, _ in anomaly_counts])
    ).update({"anomaly_count": 0}, synchronize_session=False)
